...
def get_course_details(
	...
	include_instructor_notes: int = 0,
	enforce_access: int = 0,
):
	...
	Query params:
		...
		enforce_access: 0 (default) returns the complete course regardless of
			enrolment. Pass 1 for public or unauthenticated callers to apply LMS
			access rules - unpublished courses 403, and lessons not marked
			`include_in_preview` return `content_locked: true` with empty media
			for users who are not enrolled, instructors or moderators.
	"""
	return _respond(
		course=course,
...
def _respond(course: str, include_content: bool, include_instructor_notes: bool, enforce_access: bool):
